...
bool LUABackend::updateDNSSECOrderAndAuth(uint32_t domain_id, const std::string& zonename, const std::string& qname, bool auth) {
    ...
    if(lua_pcall(lua, 4, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(updateDNSSECOrderAndAuth)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::updateDNSSECOrderAndAuthAbsolute(uint32_t domain_id, const std::string& qname, const std::string& ordername, bool auth) {
    ...
    if(lua_pcall(lua, 4, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(updateDNSSECOrderAndAuthAbsolute)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::getBeforeAndAfterNamesAbsolute(uint32_t id, const std::string& qname, std::string& unhashed, std::string& before, std::string& after) {
    ...
    if(lua_pcall(lua, 2, 3, f_lua_exec_error) != 0) {
        string e = backend_name + "(getBeforeAndAfterNamesAbsolute)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
    ...
    if (returnedwhat != LUA_TSTRING) 
        L << Logger::Error << backend_name << "(getBeforeAndAfterNamesAbsolute) ERROR! 'unhashed' is not a string." << endl;
    else
    ...
    if (returnedwhat != LUA_TSTRING) 
        L << Logger::Error << backend_name << "(getBeforeAndAfterNamesAbsolute) ERROR! 'before' is not a string." << endl;
    else
    ...
    ok = (returnedwhat == LUA_TSTRING) && ok;
    if (returnedwhat != LUA_TSTRING) 
        L << Logger::Error << backend_name << "(getBeforeAndAfterNamesAbsolute) ERROR! 'after' is not a string." << endl;
    else
        after = lua_tostring(lua, -1);
    lua_pop(lua, 1);

    if(logging)
        L << Logger::Info << backend_name << "(getBeforeAndAfterNamesAbsolute) END unhashed: '" << unhashed << "' before: '" << before << "' after: '" << after << "' ok: '" << ok << "'" << endl;
...
bool LUABackend::updateDomainKey(const string& name, unsigned int &id, bool toowhat ) {
    ...
    if(lua_pcall(lua, 3, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(updateDomainKey)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::activateDomainKey(const string& name, unsigned int id) {
    ...
    if(lua_pcall(lua, 2, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(activateDomainKey)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::deactivateDomainKey(const string& name, unsigned int id) {
    ...
    if(lua_pcall(lua, 2, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(deactivateDomainKey)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::removeDomainKey(const string& name, unsigned int id) {
    ...
    if(lua_pcall(lua, 2, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(removeDomainKey)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
//key = id, flags, active, content
    ...
    if(logging)
        L << Logger::Info << backend_name << "(addDomainKey) BEGIN name: '" << name << endl;
    ...
    if(lua_pcall(lua, 2, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(addDomainKey)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::getDomainKeys(const string& name, unsigned int kind, std::vector<KeyData>& keys) {
    ...
    if(lua_pcall(lua, 2, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(getDomainKeys)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
    ...
    if (returnedwhat != LUA_TTABLE) {
        lua_pop(lua, 1);
        if(logging)
            L << Logger::Info << backend_name << "(getDomainKeys) END - Didn't get a table as expected return value" << endl;
        return false;
    }
    ...
    while (lua_next(lua, -2)) {
        ...
        if (returnedwhat == LUA_TTABLE) {
            ...
            i = gVFT::getValueFromTable(lua, "id", kd.id);
            f = gVFT::getValueFromTable(lua, "flags", kd.flags);
            a = gVFT::getValueFromTable(lua, "active", kd.active);
            c = gVFT::getValueFromTable(lua, "content", kd.content);
    ...
    if(logging)
        L << Logger::Info << backend_name << "(getDomainKeys) END - with '" << j << "' value(s)" << endl;
...
bool LUABackend::getTSIGKey(const string& name, string* algorithm, string* content) { 
    ...
    if(lua_pcall(lua, 1, 2, f_lua_exec_error) != 0) {
        string e = backend_name + "(getTSIGKey)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::setDomainMetadata(const string& name, const std::string& kind, std::vector<std::string>& meta) {
    ...
    if(lua_pcall(lua, 3, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(setDomainMetadata)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::getDomainMetadata(const string& name, const std::string& kind, std::vector<std::string>& meta) {
    ...
    if(lua_pcall(lua, 2, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(getDomainMetadata)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
    ...
    if (returnedwhat != LUA_TTABLE) {
        lua_pop(lua,1);
        if(logging)
            L << Logger::Info << backend_name << "(getDomainMetadata) END - Didn't get a table as expected return value" << endl;
        return false;
    }
    ...
    if(logging)
        L << Logger::Info << backend_name << "(getDomainMetadata) END - with '" << j << "' value(s)" << endl;
...
void LUABackend::alsoNotifies(const string &domain, set<string> *ips) {
    ...
    if(lua_pcall(lua, 1, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(alsonotifies)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
    ...
    if (returnedwhat != LUA_TTABLE) {
        lua_pop(lua,1);
        if(logging)
            L << Logger::Info << backend_name << "(alsonotifies) END - Didn't get a table as expected return value" << endl;
        return;
    }
    ...
    if(logging)
        L << Logger::Info << backend_name << "(alsoNotifies) END - with '" << j << "' value(s)" << endl;
