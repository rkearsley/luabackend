...
bool LUABackend::startTransaction(const string &qname, int id) {
    ...
    if(lua_pcall(lua, 2, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(startTransaction)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::commitTransaction() {
    ...
    if(lua_pcall(lua, 0, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(commitTransaction)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::abortTransaction() {
    ...
    if(lua_pcall(lua, 0, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(abortTransaction)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::feedRecord(const DNSResourceRecord &rr) {
    ...
    if(lua_pcall(lua, 1, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(feedRecord)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
void LUABackend::setFresh(int id) {
    ...
    if(lua_pcall(lua, 1, 0, f_lua_exec_error) != 0) {
        string e = backend_name + "(setFresh)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
void LUABackend::getUnfreshSlaveInfos(vector<DomainInfo>* domains) {
    ...
    if(lua_pcall(lua, 0, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(getUnfreshSlaveInfos)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::isMaster(const string &domain, const string &ip) {
    ...
    if(lua_pcall(lua, 2, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(isMaster)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
...
bool LUABackend::getDomainInfo(const string &domain, DomainInfo &di) {
    ...
    if(lua_pcall(lua, 1, 1, f_lua_exec_error) != 0) {
        string e = backend_name + "(getDomainInfo)" + lua_tostring(lua, -1);
        lua_pop(lua, 1);
    ...
    if (returnedwhat != LUA_TTABLE) {
        lua_pop(lua, 1 );
        if (logging)
            L << Logger::Info << backend_name << "(getDomainInfo) END - Didn't get a table as a expected return value" << endl;
        return false;
    }
